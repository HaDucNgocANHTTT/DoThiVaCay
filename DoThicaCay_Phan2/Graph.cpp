// Phan 2: tao lop Graph cho 2 do thi vo huong, co huong va cac thuoc tinh
// Ha Duc Ngoc - ANHTTT56
#include <iostream>
#include <stdlib.h>
using namespace std;
...
void Graph::VoHuong()
{
    ...
    for (i = 0; i < row; i++)
    {
        for (j = 0; j < col; j++)
        {
            fscanf(fp, "%d ", &matran[i][j]); // Ha Duc Ngoc
        }
        fscanf(fp, "\n");
    }
    do
    {
        ...
        case 3:
            system("clear");
            cout << "-=-=-=-=-=- Danh sach ke -=-=-=-=-=-" << endl; // Ha Duc Ngoc
            cout << endl;
            for (i = 0; i < row; i++)
            {
                printf("%d | ", (i + 1));
...
void Graph::CoHuong()
{
    ...
    do
    {
        ...
        int c;
        cout << endl;
        cout << "-=-=-=-=-=- Bieu dien Do Thi CO HUONG -=-=-=-=-=-" << endl; // Ha Duc Ngoc
        cout << "1. Ma tran ke " << endl;
